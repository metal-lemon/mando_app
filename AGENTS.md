...
1. **Download Wikipedia dump**: Run the Python script to download and process the Chinese Wikipedia dump:
   ```bash
   python scripts/build_wiki_index.py --download
   ```

2. **Index generation**: The script parses ~1.4 million Wikipedia pages, extracts Chinese characters, and builds an inverted index.
